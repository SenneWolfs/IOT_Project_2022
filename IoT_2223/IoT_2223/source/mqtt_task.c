...
void mqtt_client_task(void *pvParameters)
{
     ...
     * upon failure.
     */
    printf("Task MQTT Client: Initializing Wi-Fi Connection Manager.\r\n");
    if (CY_RSLT_SUCCESS != cy_wcm_init(&config))
    {
        printf("\nTask MQTT Client: Wi-Fi Connection Manager initialization failed!\n");
        goto exit_cleanup;
    }
    ...
    printf("\nTask MQTT Client: Wi-Fi Connection Manager initialized.\n");
    ...
    {
        printf("Task MQTT Client: Failed to create Publisher task!\n");
        goto exit_cleanup;
    }

    print_heap_usage("Task MQTT Client: mqtt_client_task: subscriber & publisher tasks created");
            ...
            switch(mqtt_status)
            {
                ...
                case HANDLE_DISCONNECTION:
                {
                    ...
                    if (cy_wcm_is_connected_to_ap() == 0)
                    {
                        ...
                        printf("Task MQTT Client: Initiating Wi-Fi Reconnection...\n");
                        if (CY_RSLT_SUCCESS != wifi_connect())
                        {
                            goto exit_cleanup;
                        }
                    ...
                    printf("Task MQTT Client: Initiating MQTT Reconnection...\n");
     ...
     */
    exit_cleanup:
    printf("\nTask MQTT Client: Terminating Publisher and Subscriber tasks...\n");
...
static cy_rslt_t wifi_connect(void)
{
    ...
    if (cy_wcm_is_connected_to_ap() == 0)
    {
        ...
        printf("\nTask MQTT Client: Connecting to Wi-Fi AP '%s'\n\n", connect_param.ap_credentials.SSID);
        ...
        for (uint32_t retry_count = 0; retry_count < MAX_WIFI_CONN_RETRIES; retry_count++)
        {
            ...
            {
                printf("\nTask MQTT Client: Successfully connected to Wi-Fi network '%s'.\n", connect_param.ap_credentials.SSID);
                ...
                {
                    printf("Task MQTT Client: IPv4 Address Assigned: %s\n\n", ip4addr_ntoa((const ip4_addr_t *) &ip_address.ip.v4));
                }
                else if (ip_address.version == CY_WCM_IP_VER_V6)
                {
                    printf("Task MQTT Client: IPv6 Address Assigned: %s\n\n", ip6addr_ntoa((const ip6_addr_t *) &ip_address.ip.v6));
                }
                return result;
            }

            printf("Task MQTT Client: Connection to Wi-Fi network failed with error code 0x%0X. Retrying in %d ms. Retries left: %d\n",
        ...
        printf("\nTask MQTT Client: Exceeded maximum Wi-Fi connection attempts!\n");
        printf("Task MQTT Client: Wi-Fi connection failed after retrying for %d mins\n\n",
...
static cy_rslt_t mqtt_init(void)
{
    ...
    result = cy_mqtt_init();
    CHECK_RESULT(result, LIBS_INITIALIZED, "Task MQTT Client: MQTT library initialization failed!\n\n");
    ...
    CHECK_RESULT(result, BUFFER_INITIALIZED, "Task MQTT Client: Network Buffer allocation failed!\n\n");
    ...
    result = cy_mqtt_create(mqtt_network_buffer, MQTT_NETWORK_BUFFER_SIZE,
                            ...
                            &mqtt_connection);
    CHECK_RESULT(result, MQTT_INSTANCE_CREATED, "Task MQTT Client: MQTT instance creation failed!\n\n");
    printf("Task MQTT Client: MQTT library initialization successful.\n\n");
...
static cy_rslt_t mqtt_connect(void)
{
    ...
    CHECK_RESULT(result, 0, "Task MQTT Client: Failed to generate unique client identifier for the MQTT client!\n");
#endif /* GENERATE_UNIQUE_CLIENT_ID */
    ...
    printf("\nTask MQTT Client: MQTT client '%.*s' connecting to MQTT broker '%.*s'...\n\n",
    ...
    for (uint32_t retry_count = 0; retry_count < MAX_MQTT_CONN_RETRIES; retry_count++)
    {
        if (cy_wcm_is_connected_to_ap() == 0)
        {
            printf("Task MQTT Client: Unexpectedly disconnected from Wi-Fi network! Initiating Wi-Fi reconnection...\n");
            status_flag &= ~(WIFI_CONNECTED);
        ...
        if (result == CY_RSLT_SUCCESS)
        {
            printf("\nTask MQTT Client: MQTT connection successful.\n\n");
        ...
        printf("Task MQTT Client: MQTT connection failed with error code 0x%0X. Retrying in %d ms. Retries left: %d\n",
    ...
    printf("\nTask MQTT Client: Exceeded maximum MQTT connection attempts\n");
    printf("Task MQTT Client: MQTT connection failed after retrying for %d mins\n\n",
...
void mqtt_event_callback(cy_mqtt_t mqtt_handle, cy_mqtt_event_t event, void *user_data)
{
    ...
    switch(event.type)
    {
        case CY_MQTT_EVENT_TYPE_DISCONNECT:
        {
             ...
             */
            printf("\nTask MQTT Client: Unexpectedly disconnected from MQTT broker!\n");
        ...
        {
            /* Unknown MQTT event */
            printf("\nTask MQTT Client: Unknown Event received from MQTT callback!\n");
            break;
        }
    }
}
...
static void cleanup(void)
{
    ...
    {
        printf("Task MQTT Client: Disconnecting from the MQTT Broker...\n");
        cy_mqtt_disconnect(mqtt_connection);
    }
    ...
    if (status_flag & WIFI_CONNECTED)
    {
        ...
        {
            printf("Task MQTT Client: Disconnected from the Wi-Fi AP!\n");
        }
    }
