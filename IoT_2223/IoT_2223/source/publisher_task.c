...
void publisher_task(void *pvParameters)
{
    ...
    while (true)
    {
    	xQueueReceive(queue_battery_handle, &val_bat, portMAX_DELAY );
    	xQueueReceive(queue_speed_handle, &val_speed, portMAX_DELAY );
    	xQueueReceive(queue_servo_handle, &val_serv, portMAX_DELAY );
        	sprintf(json, "{\"batteryLevel\":%d,\"Speed\":%d,\"Servo\":%d}", val_bat, val_speed, val_serv); //Met security: 0000000
        	...
        	for (size_t i = 0; i < (aes_block_count * AES128_ENCRYPTION_LENGTH); i++)
        	{
        	...
        	publish_info.payload = json;
        	publish_info.payload_len = strlen(json);
